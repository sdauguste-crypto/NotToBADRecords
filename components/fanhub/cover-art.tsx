...
const PALETTE = ["#ff2e88", "#1e6fff", "#ff54b8", "#22d3ee", "#12224d"];
...
function Cassette({ accent }: { accent: string }) {
  return (
    <svg viewBox="0 0 100 100" className="h-3/5 w-3/5" aria-hidden>
      ...
      <circle cx="36" cy="44" r="5.5" fill="none" stroke="#22d3ee" strokeWidth="3" />
      <circle cx="64" cy="44" r="5.5" fill="none" stroke="#22d3ee" strokeWidth="3" />
    ...
    >
      ...
      <div className="relative flex h-full w-full items-center justify-center drop-shadow-[0_0_14px_rgba(4, 7, 15,.6)]">
        {motif === 0 ? (
          ...
          <RingedPlanet accent="#1e6fff" />
        ) : (
          <Cassette accent="#04070f" />
        )}
      </div>
    </div>
  );
